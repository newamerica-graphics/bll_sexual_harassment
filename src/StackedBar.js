...
export default withTooltip(props => {
  const { width, height, data, tooltipData, title } = props;
    ...
    <ParentSize>
      {({ width }) => {
        ...
        const yScale = scaleBand({
          ...
          domain: data.sort((a,b) => a.number_full_time - b.number_full_time).map(y),
          padding: 0.2
        });
      ...
      {props.tooltipOpen &&
        <Tooltip left={props.tooltipLeft} top={props.tooltipTop} style={{ borderRadius: 0 }}>
          <div>
            <h4 className="margin-top-0 margin-bottom-10" style={{ borderBottom: '1px solid rgba(0,0,0,0.15)', paddingBottom: '5px'}}>{tooltipData.data.occupation}</h4>
            <h5 className="margin-5">% Women</h5>
            <h6 className="margin-top-0 margin-bottom-10">{perFormat(tooltipData.data.percent_women)}</h6>
            <h5 className="margin-5">Median Earning</h5>
            <h6 className="margin-top-0 margin-bottom-10">{yFormat(tooltipData.data.median_earning)}</h6>
            <h5 className="margin-5"># of Workers</h5>
            <h6 className="margin-top-0 margin-bottom-10">{xFormat(tooltipData.data.number_full_time)}</h6>
          </div>
        </Tooltip>}
      </React.Fragment>
    )}}
